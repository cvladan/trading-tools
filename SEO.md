...
The sitemap contains the indexable public pages and follows the content collection as tools are added. Installation, about and licensing guidance remains public and indexable. The custom 404 uses `noindex`, has its own description and omits the misleading canonical URL previously attached to every missing address. Robots rules allow crawling, including access to these directives and source downloads.
...
Validation passed on 22 September 2026: Astro diagnostics reported no errors, warnings or hints; the build, catalogue checks and SEO checks passed. The native Cloudflare runtime was also checked locally. After publication, every generated public page and the tested SEO assets matched the build, HTTP returned 301 to HTTPS, noncanonical HTML paths returned 308, and unknown routes returned 404. The alternate `workers.dev` address redirected to the production URL. All current source downloads and the full licence matched the build. The homepage and preview image returned 200 with the tested Googlebot, Twitterbot and Facebook crawler User-Agent strings; this does not verify genuine crawler network access. Heading order and diagram accessibility labels were checked in generated HTML, and the social image was inspected visually. The development server on port 4322 remained available.

Published implementation: commit `4dfa10e`, Cloudflare version `92789dfd-dcc4-48ef-921b-3421025b6cf1`. Subsequent edits to this audit document do not change the deployed assets.
